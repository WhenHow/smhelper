...
def test_session_status_api_rejects_unknown_session_status(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        response = client.post(
            "/api/live/sessions/session-1/status",
            json={"status": "waitng", "failure_reason": None},
        )
    ...
    with Session(engine) as session:
        session_record = session.get(AccountLiveSessionRecord, "session-1")
        assert session_record is not None
        assert session_record.status == "starting"
        assert session_record.active_slot_key == "live-1:account-1"
    engine.dispose()
...
def test_session_status_api_clears_send_start_time_for_terminal_report(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "smhelper.db"
    engine = create_engine_from_url(f"sqlite+pysqlite:///{database_path}")
    ...
    with Session(engine) as session:
        session_record = session.get(AccountLiveSessionRecord, "session-1")
        assert session_record is not None
        assert session_record.status == "failed"
        assert session_record.active_slot_key is None
        assert session_record.closed_at == now.replace(tzinfo=None)
...
def test_send_result_api_rejects_unknown_send_status(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "smhelper.db"
    engine = create_engine_from_url(f"sqlite+pysqlite:///{database_path}")
    ...
    with Session(engine) as session:
        ...
        assert session_record is not None
        assert session_record.status == "sending"
        assert account is not None
...
def test_send_result_api_ignores_result_for_non_sending_session(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "smhelper.db"
    engine = create_engine_from_url(f"sqlite+pysqlite:///{database_path}")
...
def test_send_result_api_ignores_result_for_terminal_session(
    ...
    database_path = tmp_path / "smhelper.db"
    engine = create_engine_from_url(f"sqlite+pysqlite:///{database_path}")
    lost_at = datetime(2026, 6, 2, 10, 0, tzinfo=UTC)
    report_at = datetime(2026, 6, 2, 10, 5, tzinfo=UTC)
    app = create_app(
        engine=engine,
        admin_credentials=AdminCredentials(
    ...
    with Session(engine) as session:
        ...
        assert job.finished_at is None
        assert session_record is not None
        assert session_record.status == "lost"
        assert session_record.active_slot_key is None
        assert session_record.closed_at == lost_at.replace(tzinfo=None)
        assert account is not None
        assert account.sends_today == 0
        assert account.cooldown_until is None
        assert attempts == []
...
def test_send_result_api_preserves_last_successful_send_time_on_failure(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "smhelper.db"
    engine = create_engine_from_url(f"sqlite+pysqlite:///{database_path}")
    now = datetime(2026, 6, 2, 10, 0, tzinfo=UTC)
    previous_send_at = datetime(2026, 6, 2, 9, 0, tzinfo=UTC)
    app = create_app(
        engine=engine,
        admin_credentials=AdminCredentials(
            username="admin",
    ...
    with Session(engine) as session:
        ...
        assert session_record is not None
        assert session_record.status == "waiting"
        assert session_record.last_send_at == previous_send_at.replace(tzinfo=None)
        assert session_record.cooldown_until is None
        assert account is not None
        assert account.sends_today == 1
        assert account.cooldown_until is None
...
def test_send_result_api_rejects_inconsistent_job_session_owner(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "smhelper.db"
    engine = create_engine_from_url(f"sqlite+pysqlite:///{database_path}")
    ...
    with Session(engine) as session:
        job = session.get(DispatchJobRecord, "job-1")
        session_record = session.get(AccountLiveSessionRecord, "session-1")
        account_1 = session.get(PlatformAccountRecord, "account-1")
        account_2 = session.get(PlatformAccountRecord, "account-2")
        attempts = session.query(SendAttemptRecord).all()
        assert job is not None
        assert job.status == "running"
        assert job.finished_at is None
        assert session_record is not None
        assert session_record.status == "sending"
        assert session_record.account_id == "account-2"
        assert account_1 is not None
        assert account_1.sends_today == 0
        assert account_2 is not None
        assert account_2.sends_today == 0
        assert attempts == []
    engine.dispose()
